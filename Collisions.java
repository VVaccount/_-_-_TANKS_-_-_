    ...
    public void shellsCollisions(Player player, Group root, Enemy enemy) {
        ...
        if (player.shooted) {
            ...
            if (shootUp || shootDown || shootLeft || shootRight) {
                ...
                if (!shootUpCollision && !shootDownCollision && !shootLeftCollision && !shootRightCollision) {
                    player.shells.add(new ImageView(new Image(Tanks.class.getResourceAsStream("sprites.jpg"), 800, 512, false, true)));
                    if (shootUp) {
                ...
                shootUpCollision = false;
                shootDownCollision = false;
                shootLeftCollision = false;
                shootRightCollision = false;
            }
        }
        ...
        while (shellsIterator.hasNext()) {
            ...
            while (bricksIterator.hasNext()) {
                ...
                if (bufferShell.intersects(bufferBrick.getX(), bufferBrick.getY(), map.size, map.size)) {
                    ...
                    root.getChildren().remove(bufferBrick);
                    root.getChildren().remove(bufferShell);
                    map.tiles.remove(bufferBrick);
                    map.tiles.add(map.brickTiles.indexOf(bufferBrick), new ImageView(new Image(Tanks.class.getResourceAsStream("imgonline-com-ua-Transparent-backgr-5KGwsJZpkwVzR71.png"), 800, 512, false, true)));
                    map.tiles.get(map.brickTiles.indexOf(bufferBrick)).setViewport(new Rectangle2D(map.spaceOffsetX, map.spaceOffsetY, map.size, map.size));
                    map.tiles.get(map.brickTiles.indexOf(bufferBrick)).setX(map.getCoordinate((int) (map.brickTiles.get(map.brickTiles.indexOf(bufferBrick)).getX() - map.startX) / map.size, map.size, map.startX));
                    map.tiles.get(map.brickTiles.indexOf(bufferBrick)).setY(map.getCoordinate((int) (map.brickTiles.get(map.brickTiles.indexOf(bufferBrick)).getY() - map.startY) / map.size, map.size, map.startY));
                    root.getChildren().add(map.tiles.get(map.brickTiles.indexOf(bufferBrick)));
    ...
    public void enemyShellsCollisions(Enemy enemy, Group root, Player player) {
        ...
        if (enemy.shooted) {
            ...
            if (shootUp || shootDown || shootLeft || shootRight) {
                ...
                if (!shootUpCollision && !shootDownCollision && !shootLeftCollision && !shootRightCollision) {
                    enemy.shells.add(new ImageView(new Image(Tanks.class.getResourceAsStream("sprites.jpg"), 800, 512, false, true)));
                    if (shootUp) {
...
//                    }
    ...
    public void deleteBrick(int brickNumber, Group root) {
        root.getChildren().remove(map.brickTiles.get(brickNumber));
        map.tiles.remove(map.brickTiles.get(brickNumber));
        map.tiles.add(brickNumber, new ImageView(new Image(Tanks.class.getResourceAsStream("imgonline-com-ua-Transparent-backgr-5KGwsJZpkwVzR71.png"), 800, 512, false, true)));
        map.tiles.get(brickNumber).setViewport(new Rectangle2D(map.spaceOffsetX, map.spaceOffsetY, 16, 16));
        map.tiles.get(brickNumber).setX(map.getCoordinate((int) (map.brickTiles.get(brickNumber).getX() - map.startX) / map.size, map.size, map.startX));
        map.tiles.get(brickNumber).setY(map.getCoordinate((int) (map.brickTiles.get(brickNumber).getY() - map.startY) / map.size, map.size, map.startY));
        root.getChildren().add(map.tiles.get(brickNumber));
