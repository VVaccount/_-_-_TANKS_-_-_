...
public class Player {
    ...
    Player(Group root, Replay replay) {
        shells = new ArrayList<>();
        offsetX = 0;
        offsetY = 0;
        playerTank = new ImageView(new Image(Tanks.class.getResourceAsStream("sprites.jpg"), 800, 512, false, true));
        ...
        leftCollision = false;
        rightCollision = false;
        setPlayerEvents(root, replay);
    }

    public void update(Collisions collisions, Group root, String controlLine) {
        if (controlLine.equals("PLAYER MOVED UP")) {
            up = playerTank.getY() > 182;
            down = false;
            left = false;
            right = false;
        }
        if (controlLine.equals("PLAYER MOVED DOWN")) {
            up = false;
            down = playerTank.getY() < 566;
            left = false;
            right = false;
        }
        if (controlLine.equals("PLAYER MOVED LEFT")) {
            up = false;
            down = false;
            left = playerTank.getX() > 442;
            right = false;
        }
        if (controlLine.equals("PLAYER MOVED RIGHT")) {
            up = false;
            down = false;
            left = false;
            right = playerTank.getX() < 826;
        }
        if (controlLine.equals("PLAYER SHOOTED")) {
            up = false;
            down = false;
            left = false;
            right = false;
    ...
    public void setPlayerEvents(Group root, Replay replay) {
        movement = new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent keyEvent) {
                ...
                if (exit) {
                    replay.writeReplayFile("END");
                    try {
                        replay.replayWriter.close();
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    Platform.exit();
    ...
    public void update(Collisions collisions, Group root, Replay replay) {
        ...
        collisions.tankCollisions(this, enemy, replay);
        collisions.shellsCollisions(this, root, enemy, replay);
    }
